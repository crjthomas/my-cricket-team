    ...
    createPlayer: async (_: unknown, { input }: { input: Record<string, unknown> }) => {
      const player = await prisma.player.create({
        data: {
          ...
          isCaptain: (input.isCaptain as boolean) || false,
          isViceCaptain: (input.isViceCaptain as boolean) || false,
          availableForT20: input.availableForT20 !== undefined ? (input.availableForT20 as boolean) : true,
          availableForT30: input.availableForT30 !== undefined ? (input.availableForT30 as boolean) : true,
          leaguePreferenceNotes: input.leaguePreferenceNotes as string | undefined,
          fitnessLevel: (input.fitnessLevel as number) || 5,
          currentInjuryStatus: (input.currentInjuryStatus as 'FIT' | 'MINOR_NIGGLE' | 'RECOVERING' | 'INJURED') || 'FIT',
          powerHitting: (input.powerHitting as number) || 5,
          runningBetweenWickets: (input.runningBetweenWickets as number) || 5,
          pressureHandling: (input.pressureHandling as number) || 5,
          reliabilityScore: (input.reliabilityScore as number) || 5,
          trainingAttendance: input.trainingAttendance as number | undefined,
          preferredFieldingPositions: (input.preferredFieldingPositions as string[]) || [],
          bowlingVariations: (input.bowlingVariations as string[]) || [],
          previousTeams: (input.previousTeams as string[]) || [],
          injuryHistory: (input.injuryHistory as string[]) || [],
          isRookie: (input.isRookie as boolean) || false,
          tennisBallBackground: (input.tennisBallBackground as boolean) || false,
          yearsPlaying: input.yearsPlaying as number | undefined,
        },
      })
